 ...
 *
 * Leaves are found by the gap between them: a reveal is 3-4mm and no two
 * triangles of the same leaf are ever that far apart along x without something
 * between them. One cluster returns the group untouched, so a slab front and
 * every non-door group cost nothing.
 */
export function splitDoorLeaves(group: MeshGroup, gapMm = 3): MeshGroup[] {
