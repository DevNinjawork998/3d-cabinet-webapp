...
### The one place a mesh is loaded

`components/admin/DesignViewer.tsx` loads an uploaded `.obj` with three's own
`OBJLoader` and shows it in an orbitable canvas. That is not the rule above
being broken — it is the rule's scope. The no-loaded-models rule protects the
**customer-facing planner**, where a baked mesh cannot resize to a size ladder,
carries no parameters so can never yield a price or a BOM, and blows the mobile
budget. None of that applies to an admin behind a login checking that the file
they just attached is the cabinet they meant, before anything is priced or
published — a question a table of numbers cannot answer.

It is kept honest by construction: only `/admin/cabinet-designs` imports it,
behind `next/dynamic` with `ssr: false`, so the loader never enters a bundle a
customer downloads. Keep it that way — if a viewer is ever wanted on a public
page, that is a new decision, not an extension of this one.
